...
<body>
	...
	<h1 class="Lanbra">
		<img class=logo src="resources/img/logo.png" alt="Image"> Lanbra
		job
	</h1>
		...
		<tbody>
			<c:forEach var="industry" items="${industrys}">
				<tr>
					...
					<td> <button onclick="openDeleteModal('${industry.id}')">Delete</button></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
	...
	<div class="txtpos">
		...
		<form:form action="addsimpleindustry" method="post"
			modelAttribute="industry">
			<div class="loginclass" id="position">
				<div>
					<h2 class="header">Add New Industry</h2>
				</div>

				<div class="errorcolor">${error.allErrors}</div>

				<div>
					<form:input path="name" placeholder="Enter Industry"></form:input>
	...
	<script>
    var industryIdToDelete;

    function openDeleteModal(id) {
      industryIdToDelete = id;
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
        ...
        fetch('deleteindustry/' + industryIdToDelete, {
            method: 'GET'
        })
        .then(response => {
            if (response.ok) {
