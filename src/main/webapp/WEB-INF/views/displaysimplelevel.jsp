...
/* right form css */
      .header {
                background-color: #333; color: #00FF00; text-align: center; padding: 10px 0; border-radius: 12px;
            }
            .loginclass{
                width: 20%;
                margin: 0 auto;
		...
		<tbody>
			<c:forEach var="level" items="${levels}">
				<tr>
					...
					<td> <button onclick="openDeleteModal('${level.id}')">Delete</button></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
...
<div class="txtpos">
	<form:form action="addsimplelevel" method="post"
			modelAttribute="level">
			<div class="loginclass" id="position">
				<div>
					<h2 class="header">Add New Level</h2>
				</div>

				<div class="errorcolor">${error.allErrors}</div>

				<div>
					<form:input path="name" placeholder="Enter Level"></form:input>
...
<script>
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
        ...
        fetch('deletesimplelevel/' + levelIdToDelete, {
            method: 'GET'
        })
        .then(response => {
            if (response.ok) {
